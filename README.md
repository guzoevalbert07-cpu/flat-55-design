...
Секция «Визуализации квартиры» — 16 видов на опцию: вход, коридор, с/у, душевая (на основе кадров видео — плитка и подиум сохраняются), кухня,
кухня крупно, обеденная зона, зал (2 вида), спальня (2 вида), лоджия, стены/потолок, вариант «детская», двери. Генерация — FLUX.1 через публичные
Hugging Face Spaces (`black-forest-labs/FLUX.1-schnell` для текстовых промптов, `black-forest-labs/FLUX.1-Kontext-Dev` для image-to-image на кадрах
видео), ключ не нужен: `python3 scripts/render_views.py <папка с кадрами f_*.jpg>` (окружение: `pip install gradio_client pillow`). Кадры из видео:
`ffmpeg -i видео.mp4 -vf fps=2,scale=960:-1 f_%03d.jpg`. Скрипт возобновляемый; `--views wc,shower --force` перегенерирует выбранные виды.
Манифест — `src/data/renders.json` (промпт и seed каждой картинки). Это иллюстрации замысла, не чертежи — так и подписано на сайте.
