...
| `Дизайн-концепция_55м2_3_опции.docx` | Планировочное решение, три опции, детализация по комнатам | Тексты в `src/data/options.ts`; схема из docx (`input/plan.png`) не используется — геометрия плана по фото БТИ `public/plan-bti.jpg` |
...
Секция «Визуализации квартиры» — 16 видов на опцию. Вход, коридор, с/у, душевая, ванная — **один общий набор для всех опций** (`public/renders/shared-*.jpg`,
по указанию заказчика: плитка там уже уложена, видно на видео — керамогранит у входа, ониксовая плитка, ниша и подиум душа; описание отделки зашито в промпт).
Остальные 11 видов — по опциям: кухня, кухня крупно, обеденная зона, зал (2 вида), спальня (2 вида), лоджия, стены/потолок, вариант «детская», двери.
Промпты и список видов — `scripts/render_prompts.py`.
...
Скрипт возобновляемый (готовые файлы пропускает), на «censored» от воркера повторяет с другой формулировкой; общие виды генерирует один раз как `shared-<вид>.jpg`
и записывает в манифест по одной записи на опцию (`shared: true`). Масштаб: FLUX по умолчанию рисует залы по 30 м², поэтому в каждый промпт зашиты реальные
размеры комнаты с плана БТИ (`SCALE` + размеры в `VIEWS_DEF`), объектив 35 мм и «проходы 60–70 см»; узкие комнаты (`PORTRAIT_VIEWS`) рисуются в портрете;
для не-кухонных видов стиль без кухонных фасадов (`STYLE_ROOM`), иначе модель тащит кухонные шкафы в спальню. Кадры из видео:
