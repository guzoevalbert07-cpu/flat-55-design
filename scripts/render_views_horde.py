...
Запуск: python3 scripts/render_views_horde.py <папка с кадрами f_*.jpg> [--only eco,std] [--views wc,shower] [--force] [--parallel 3] [--seed-shift 1000]
...
from render_prompts import VIEWS_DEF, PORTRAIT_VIEWS, build_prompt, build_video_prompt  # noqa: E402
...
SEED_SHIFT = int(args[args.index("--seed-shift") + 1]) if "--seed-shift" in args else 0  # перегенерация с другим seed
...
def generate(prompt, kind, frame, seed, key=None):
    # портрет: виды по видео (узкие санузлы и коридор) и узкие комнаты из PORTRAIT_VIEWS — так квадратура читается честнее
    portrait = kind == "kontext" or key in PORTRAIT_VIEWS
...
def worker(q):
    while True:
        ...
        seed = 100 + i + (900 if key == "corridor" else 0) + SEED_SHIFT  # seed 102 для коридора стабильно давал мусорный текст на стене
        t = time.time()
        for attempt in range(5):
            try:
                ...
                url, model, wname = generate(p_try, kind, FRAMES / frame if frame else None, seed + 500 * attempt, key)
