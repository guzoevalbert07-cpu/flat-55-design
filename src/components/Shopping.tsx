    ...
    <section className="section" id="shopping" aria-labelledby="shop-h">
      <div className="wrap">
        ...
        <p className="lead">
          {bench ? (
            <>
              На каждую из {total} позиций сметы — вариант для опции {opt.name} (столбец {opt.excelCol}) с ценой со страницы магазина, прайс-листа или из
              поисковой выдачи{checked ? ` на ${checked}` : ''} и рядом цена из сметы; у {confirmed} позиций цена взята прямо со страницы магазина или прайса, отклонение
              от сметы считается только для них. Это лист «Ссылки_по_позициям» в Excel; цены в самой смете не менялись. У Hoff, Ситилинка, Асконы и подобных
              показана федеральная цена без выбора города — так и подписано. Под каждой позицией — готовый поиск на Авито Саратов: через Авито можно посмотреть
              предложения и оптимизировать стоимость относительно сметы; цены с Авито смотрите вручную, автоматически они не снимаются. Перед оплатой сверяйте
              на сайте магазина.
            </>
          ) : (
            <>
                          ...
                          <div className="store">
                            ...
                            {o.status && (
                              <span className={`pill${o.status === 'ок' ? ' on' : ''}`} style={{ marginLeft: 6 }}>
                                {o.status === 'ок' ? 'цена со страницы магазина' : o.status === 'по сниппету' ? 'цена из поисковой выдачи, ориентир' : 'уточнить'}
                              </span>
                            )}
                          </div>
                    ...
                    {p.avito && (
                      <div className="avito">
                        <a href={p.avito.url} target="_blank" rel="noopener">
                          Авито Саратов — посмотреть предложения и оптимизировать стоимость (цены вручную) ↗
                        </a>
                      </div>
                    )}
                  </div>
